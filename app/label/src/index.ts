...
const allLabels = appConfig.cLabels.map(l => l.labels).flat(1);
...
async function createLabel(label: CLabel): Promise<[success: boolean, msg: string]> {
    const [err, res] = await awaitTo(labelApi.createLabel(label));
...
async function createDefaultLabels() {
    ...
    let [nErr, isNext] = await awaitTo(confirm({ message: '是否继续?' }));
    if (nErr || !isNext) {
    ...
    // 表格显示已选标签
    const labels = allLabels.filter(l => labelNames.includes(l.name));
    console.table(labels.map(l => ({ ...l, color: l.color ?? '随机' })));

    // 最后确认
    [nErr, isNext] = await awaitTo(confirm({ message: `已选择[${labelNames.length}]个标签, 是否开始创建?` }));
    if (nErr || !isNext) {
        return console.log('已取消!');
    }
...
async function createCustomLabels() {
    ...
    try {
        label = {
            name: await input({ message: '[1/3]请输入标签名称:' }),
